...
//This section is for digital pins used as chokes. This can be used with or without multiplexers, as long as the pins are not used for anything else.
//An example for the choke note/name/pin here, plus the code for the bounce library when using a Teensy. Add as many as wanted following these examples.

#if USE_DIG
 #if TEENSY
   // Define the note for the choke
  #define Choke1 78

   // Define the note for digital auxiliary choke
  #define Aux1 47

   // Define the pin for the choke
 #define Choke1_Pin 4 

   // Define the pin for the auxiliary choke
 #define Aux1_Pin 8
 
 #include <Bounce.h>
  
  Bounce Choke_A = Bounce(Choke1_Pin, 5); // 5 = 5 ms debounce time which is appropriate for good quality mechanical pushbuttons
  Bounce AUX_A = Bounce(Aux1_Pin, 5);  // if a button is too "sensitive" to rapid touch, you can increase this time.
...
//Define the initial state of the choke pins
